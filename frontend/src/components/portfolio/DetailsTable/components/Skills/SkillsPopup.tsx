'use client'
import { SKILLS_DATA } from "@/common/data/pages/portfolio"
...
export default function SkillsPopup({ initialOpenTab }: { initialOpenTab: string }) {
    const [activeCategory, setActiveCategory] = useState<string>(initialOpenTab || SKILLS_DATA[0].category)

    return (
        <section className="overflow-hidden bg-matte/40 backdrop-blur-md max-sm:border-t sm:border-l border-snow/10 rounded-t-3xl sm:rounded-none grid grid-cols-1 sm:grid-cols-[1fr_2fr] gap-x-4 w-device md:w-[750px] h-[85dvh] sm:h-device">
            <div className="max-sm:row-start-2 flex sm:flex-col justify-start max-sm:items-center max-sm:border-t border-snow/10 overflow-auto scrollbar-hide gap-2.5 sm:gap-4 max-sm:px-4 max-sm:py-2 sm:pl-5 sm:pt-8 max-sm:h-fit">
                {SKILLS_DATA.map(({ category }, index) => (
                    <div
                        ...
                        className={`${activeCategory === category ? "bg-accent/10 text-accent" : "max-sm:bg-smoke/40 sm:hover:bg-smoke/40"} cursor-pointer transition-colors duration-300 rounded-full sm:rounded-xl flex flex-col items-start gap-1 max-sm:text-sm px-4 py-1.5 sm:px-4 sm:py-4`}
                        key={index}
                    >
                        <span className="max-sm:whitespace-nowrap">{category}</span>
                        <span className={`${activeCategory === category ? "" : "text-snow/35"} max-sm:hidden text-xs transition-all duration-100`}>20 projects</span>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 auto-rows-min  max-sm:px-5 sm:py-6 sm:pl-6 sm:pr-5 bg-matte overflow-auto scrollbar-hide gap-y-6 max-sm:h-[calc(85dvh_-_50px)]">
                ...
                {/* <ImageButton
                    title="Giftlaya"
                    bottomLabel="giftlaya.com"
                    image={{ alt: "", src: "/images/projects/giftlaya-desktop.webp" }}
                    svg={
